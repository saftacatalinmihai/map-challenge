...
my $prod = container 'Prod' => as {
    container 'Mongo' => as {
        service 'host' =>
            'mongodb://admin:a@ds035250.mongolab.com:35250/mapchallenge';
        service 'port' => '35250';
    };
    container "ElasticSearch" => as {
        service 'host' =>
            'https://d85gv4pa:6vhyka1est2zmdmq@ash-4561628.eu-west-1.bonsai.io:443';
        service 'port' => '443';
    };
    alias "mongo_host"   => 'Mongo/host';
    alias "mongo_port"   => 'Mongo/port';
...
my $c = container 'MapChallange' => ['Env'] => as {
    ...
    container "Collections" => as {
        service 'places' => (
            ...
            dependencies => { dbh => '/MongoDatabase/dbh', },
            lifecycle    => 'Singleton',
            ),
    };
...
my $MapChallange
    = $parametrized_component->resolve( service => 'MapChallangeApp' );
